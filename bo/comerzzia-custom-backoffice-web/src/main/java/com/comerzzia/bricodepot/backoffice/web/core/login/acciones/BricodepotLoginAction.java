...
public class BricodepotLoginAction extends LoginAccion {
	...
	public Vista ejecutar(HttpServletRequest request) {
		...
		try {
			...
			// Iniciamos la sesión
			DatosSesionBean datosSesion = (DatosSesionBean) ServicioLoginImpl.get().iniciarSesion(usuario.toUpperCase(),
					clave, uidInstancia, uidActividad, codEmpresa, null);
			...
			if (datosSesion.getDatosEmpresa().getDesEmp() == null
					|| datosSesion.getDatosEmpresa().getDesEmp().isEmpty()) {
				return CONFIG_PAGE;
			}
